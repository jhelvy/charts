...
election_margins <- elections %>%
    mutate(
        candidate = fct_reorder(candidate, -margin_pop),
        # candidate = fct_reorder(candidate, -year), # Add this to sort by date
        candidateLabelPos = ifelse(margin_pop > 0, -0.004, 0.004),
        candidateLabelHjust = ifelse(margin_pop > 0, 1, 0),
        marginLabel = scales::percent(margin_pop, accuracy = 0.1),
        marginLabelHjust = ifelse(margin_pop > 0, 0, 1),
        marginLablePos = ifelse(margin_pop > 0, margin_pop + 0.005,
                                margin_pop - 0.005)) %>%
    ggplot() +
    geom_col(aes(x = margin_pop, y = candidate, fill = political_party)) +
...
ggsave(here::here("plots", "election_margins_pop.pdf"),
       election_margins, width = 7, height = 10, device = cairo_pdf)

ggsave(here::here("plots", "election_margins_pop.png"),
       election_margins, width = 7, height = 10)

# ggsave(here::here("plots", "election_margins_pop_by_year.pdf"),
#        election_margins, width = 7, height = 10, device = cairo_pdf)
#
# ggsave(here::here("plots", "election_margins_pop_by_year.png"),
#        election_margins, width = 7, height = 10)
