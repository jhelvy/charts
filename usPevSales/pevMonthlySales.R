...
titleLabel <- 'U.S. Monthly Sales of Plug-in Electric Vehicles, 2014 - 2020'
subtitleLabel <- 'With the exception of Tesla, combined monthly PEV sales by all other automakers has been flat for the past six years'
captionLabel <- "Data sources: hybridcars.com ('14-'17) & insideEVs.com ('18-'19)"
annLabel <- paste0('Six-year mean (non-Tesla):\n',
                   scales::comma(10^3*nonTeslaMean), ' PEV sales per month')

pevMonthlySales <- pevSales %>%
    ggplot() +
    ...
    geom_curve(
        aes(x = ymd('2014-12-01'), xend = ymd('2015-02-01'), y = 18, yend = 10),
        size = 0.2, curvature = 0.1,
        arrow = arrow(length = unit(0.02, "npc"), type = "closed")) +
    geom_label(aes(x = ymd('2014-06-01'), y = 20), label = annLabel,
        hjust = 0, lineheight = 1, family = mainFont, size = 4.5)

# Save as PDF and PNG files
ggsave(here::here('plots', 'pevMonthlySales.pdf'), pevMonthlySales,
       width = 10, height = 5, device = cairo_pdf)

ggsave(here::here('plots', 'pevMonthlySales.png'), pevMonthlySales,
       width = 10, height = 5)
